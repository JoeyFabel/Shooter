...
namespace DarkTreeFPS
{
    ...
    public class Weapon : MonoBehaviour
    {
        ...
        public int burstAmount = 3; // number of bullets in burst
        public bool burstEnabled = false;
        public bool automaticEnabled = true;
        ...
        private void Start()
        {
            ...
            if (weaponType == WeaponType.Melee || weaponType == WeaponType.Pistol || weaponType == WeaponType.Grenade)
            {
        ...
        void Update()
        {
            ...
            // auto mode
            if (Input.GetKey(input.Fire) && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType != WeaponType.RocketLauncher && weaponType != WeaponType.Pistol && !InventoryManager.showInventory && fireMode == FireMode.automatic)  //Statement to restrict auto-fire for pistol weapon type. Riffle and others are automatic
            {
                Fire();
            } // burst mode
            else if (Input.GetKeyDown(input.Fire) && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType != WeaponType.RocketLauncher && weaponType != WeaponType.Pistol && !InventoryManager.showInventory && fireMode == FireMode.burst)  //Statement to restrict auto-fire for pistol weapon type. Riffle and others are automatic
            {
                // Don't have multiple bursts at once!
                if (!isBurstFiring) StartCoroutine(BurstFire());
            } // single shot                   
            else if (Input.GetKeyDown(input.Fire) && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType != WeaponType.RocketLauncher && (weaponType == WeaponType.Pistol || fireMode == FireMode.single) && !InventoryManager.showInventory)
            {
                Fire();
            }
            else if (Input.GetKeyDown(input.Fire) && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType == WeaponType.RocketLauncher && !InventoryManager.showInventory)
            {
                FireRocketLauncher();
            }
            ...
            if (weaponType != WeaponType.Melee && weaponType != WeaponType.Grenade)
            {
                ...
                if (/*currentItem != null*/weaponManager.activeWeapon != null && (Input.GetKeyDown(input.Reload) || /*currentItem.stackSize*/ currentAmmo == 0))
                {
                    if (weaponType != WeaponType.Shotgun)
                    {
                        if (!reloading && !controller.isClimbing && /*currentItem.stackSize*/ currentAmmo < maxAmmo)
                            ReloadBegin();
                    }
                    else
                    {
            ...
            if (weaponType == WeaponType.Shotgun)
            {
                if (reloading && /*CalculateTotalAmmo()*/ Inventory.GetAmmo(ammoItemID) == 0)
                {
                ...
                if (/*currentItem.stackSize*/ currentAmmo == maxAmmo)
                {
                    animator.SetBool("Reloading", false);
                }
            }
        ...
        public System.Collections.IEnumerator BurstFire()
        {
            print("Burst fire!");
            int shotsFired = 0;
            isBurstFiring = true;
            while (shotsFired < burstAmount)
            {
                Fire();
        ...
        public void Fire()
        {
            if (weaponType != WeaponType.Melee && weaponType != WeaponType.Grenade && !reloading)
            {
                ...
                {
                    if (/*currentItem.stackSize*/ currentAmmo > 0)
                    {
        ...
        public void ReloadBegin()
        {
            ...
            if (/*CalculateTotalAmmo()*/ Inventory.GetAmmo(ammoItemID) > 0)
            {
                if (weaponType == WeaponType.RocketLauncher)
                {
        ...
        void ReloadEnd()
        {
            ...
            int ammoToTake = Mathf.Min(neededAmmo, Inventory.GetAmmo(ammoItemID));
            Inventory.UseBullets(ammoItemID, ammoToTake);
        ...
        private void FireModeSwitch()
        {
            ...
            {
                if (fireMode != FireMode.burst && burstEnabled) fireMode = FireMode.burst;
                else if (automaticEnabled) fireMode = FireMode.automatic;
            }
            if (Input.GetKeyDown(input.FiremodeSingle))
            {
                fireMode = FireMode.single;
            }*/
            if (Input.GetKeyDown(input.FiremodeToggle))
            {
                if (fireMode == FireMode.single)
                {
                    if (burstEnabled) fireMode = FireMode.burst;
                    else if (automaticEnabled) fireMode = FireMode.automatic;
                }
                else if (fireMode == FireMode.burst)
                {
                    if (automaticEnabled) fireMode = FireMode.automatic;
        ...
        private void UpdateAmmoText()
        {
            ...
            if (weaponType != WeaponType.Grenade)
                //if (currentItem != null)
                if (weaponManager.activeWeapon != null && weaponManager.activeWeapon.weaponType != WeaponType.Melee)
                    ammoText.text = string.Format("{0}|{1}\n Mode:{2}", /*currentItem.stackSize*/ currentAmmo, Inventory.GetAmmo(ammoItemID), mode);
        }
        ...
        public void AimFOVShift()
        {
            ...
            {                
                ...
                // should be head bob cam - if ADS not working, make sure this is the case
                //Debug.Log("Main cam", mainCamera);
            }
            else
            {
